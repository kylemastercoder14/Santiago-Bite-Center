...
export const MedicalFormValidators = z.object({
    ...
    medication: z.string().min(1, {message: "Prescribed medication is required."}),
...
export const TreatmentFormValidators = z.object({
    ...
    tetanusImmunuglobulin: z.string().min(1, {message: "Tetanus immunuglobulin is required."}),
    tetanusSerum: z.string().min(1, {message: "Tetanus serum is required."}),
    antiRabiesSerum: z.string().min(1, {message: "Anti rabies serum is required."}),
    chickEmbryoCellVaccine: z.string().min(1, {message: "Chick embryo cell vaccine is required."}),
    verocellRabiesVaccine: z.string().min(1, {message: "Verocell rabies vaccine is required."}),
})

export const VitalSignFormValidators = z.object({
    temperature: z.coerce.number().min(1, {message: "Temperature is required."}),
    weight: z.coerce.number().min(1, {message: "Weight is required."}),
    pulse:  z.coerce.number().min(1, {message: "Pulse is required."}),
    respiration: z.coerce.number().min(1, {message: "Respiration is required."}),
    bloodPressure: z.coerce.number().min(1, {message: "Blood pressure is required."}),
