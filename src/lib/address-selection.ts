...
import { getRegions, getProvinces, getMunicipalities, getBarangays } from "@/constants";

export const useAddressData = (selectedRegionName: string, selectedProvinceName: string, selectedMunicipalityName: string) => {
    ...
    useEffect(() => {
        const selectedRegion = getRegions().find((region) => region.region_description === selectedRegionName);
        setRegionId(selectedRegion?.region_id || null);
    }, [selectedRegionName]);

    useEffect(() => {
        const selectedProvince = getProvinces(regionId).find((province) => province.province_name === selectedProvinceName);
        setProvinceId(selectedProvince?.province_id || null);
    }, [selectedProvinceName, regionId]);

    useEffect(() => {
        const selectedMunicipality = getMunicipalities(provinceId).find((municipality) => municipality.municipality_name === selectedMunicipalityName);
